...
export class OcrService {
   ...
   */
  async uploadPvPolice(file: Express.Multer.File, claimId?: string): Promise<any> {
    try {
      if (!file) {
      ...
      pvPolice.claim_id = claimId ?? null;
      pvPolice.pv_number = pvData.pv_number || `PV-${Date.now()}`;
      pvPolice.accident_date = pvData.accident_date ? new Date(pvData.accident_date) : new Date();
      pvPolice.accident_time = pvData.accident_time || null;
      pvPolice.accident_location = pvData.accident_location || null;
      pvPolice.driver_a_name = pvData.driver_a_name || null;
      pvPolice.driver_a_cin = pvData.driver_a_cin || null;
      pvPolice.driver_a_permit = pvData.driver_a_permit || null;
      pvPolice.driver_b_name = pvData.driver_b_name || null;
      pvPolice.driver_b_cin = pvData.driver_b_cin || null;
      pvPolice.driver_b_permit = pvData.driver_b_permit || null;
      pvPolice.vehicle_a_registration = pvData.vehicle_a_registration || null;
      pvPolice.vehicle_b_registration = pvData.vehicle_b_registration || null;
      pvPolice.circumstances = pvData.circumstances || null;
      pvPolice.responsibility = pvData.responsibility || null;
      pvPolice.injured_persons = pvData.injured_persons || null;
      pvPolice.witnesses = pvData.witnesses || null;
      pvPolice.officer_name = pvData.officer_name || null;
      pvPolice.police_station = pvData.police_station || null;
      pvPolice.file_name = file.originalname;
      pvPolice.extracted_text = extractedText.substring(0, 10000);
      pvPolice.parsed_data = pvData;
   ...
   */
  async uploadRapportExpert(file: Express.Multer.File, claimId?: string): Promise<any> {
    try {
      if (!file) {
        throw new Error('No file uploaded');
      }
      ...
      rapportExpert.claim_id = claimId ?? null;
      rapportExpert.reference = expertData.reference || `RAP-${Date.now()}`;
      rapportExpert.expert_name = expertData.expert_name || null;
      rapportExpert.expert_agrément = expertData.expert_agrément || null;
      rapportExpert.expertise_date = expertData.expertise_date ? new Date(expertData.expertise_date) : new Date();
      rapportExpert.vehicle_make = expertData.vehicle_make || null;
      rapportExpert.vehicle_model = expertData.vehicle_model || null;
      rapportExpert.vehicle_registration = expertData.vehicle_registration || null;
      rapportExpert.vehicle_vin = expertData.vehicle_vin || null;
      rapportExpert.vehicle_mileage = expertData.vehicle_mileage || null;
      rapportExpert.vehicle_year = expertData.vehicle_year || null;
      rapportExpert.vehicle_value_new = expertData.vehicle_value_new || null;
      rapportExpert.vehicle_value_market = expertData.vehicle_value_market || null;
      rapportExpert.damages_description = expertData.damages_description || null;
      rapportExpert.parts_to_replace = expertData.parts_to_replace || null;
      rapportExpert.parts_to_repair = expertData.parts_to_repair || null;
      rapportExpert.repair_value_estimate = expertData.repair_value_estimate || null;
      rapportExpert.conclusion = expertData.conclusion || null;
      rapportExpert.file_name = file.originalname;
      rapportExpert.extracted_text = extractedText.substring(0, 10000);
      rapportExpert.parsed_data = expertData;
   ...
   */
  async uploadDevis(file: Express.Multer.File, claimId?: string): Promise<any> {
    try {
      if (!file) {
        throw new Error('No file uploaded');
      }
      ...
      devis.claim_id = claimId ?? null;
      devis.reference = devisData.reference || `DEV-${Date.now()}`;
      devis.devis_date = devisData.devis_date ? new Date(devisData.devis_date) : new Date();
      devis.garage_name = devisData.garage_name || null;
      devis.garage_address = devisData.garage_address || null;
      devis.garage_fiscal_id = devisData.garage_fiscal_id || null;
      devis.vehicle_make = devisData.vehicle_make || null;
      devis.vehicle_model = devisData.vehicle_model || null;
      devis.vehicle_registration = devisData.vehicle_registration || null;
      devis.owner_name = devisData.owner_name || null;
      devis.items_list = devisData.items_list || null;
      devis.labor_hours = devisData.labor_hours || null;
      devis.labor_rate_per_hour = devisData.labor_rate_per_hour || null;
      devis.subtotal_ht = devisData.subtotal_ht || null;
      devis.tva_19 = devisData.tva_19 || null;
      devis.total_ttc = devisData.total_ttc || null;
      devis.validity_days = devisData.validity_days || null;
      devis.file_name = file.originalname;
      devis.extracted_text = extractedText.substring(0, 10000);
      devis.parsed_data = devisData;
